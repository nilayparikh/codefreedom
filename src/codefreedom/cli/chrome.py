...
Usage:
    ...
    codefreedom tools chrome start   Start headless Chrome container (CDP port 9222)
...
This tool runs plain headless Google Chrome for browser automation.
For web search and scraping, use the 'web' tool instead — see `codefreedom tools web`.

Settings are loaded from ~/.codefreedom/profiles/chrome.json (generated by
'codefreedom tools chrome init').  Use `cf init recipe` to initialize.
"""
...
def _profile_path() -> Path:
    """Return the chrome tool profile path (re-evaluated each call so tests can
    override ``CODEFREEDOM_HOME`` after import)."""
    return get_codefreedom_dir() / "profiles" / "chrome.json"
...
def init_tool() -> int:
    ...
    print("[chrome init] Bundled examples have been replaced by recipes.")
    print()
