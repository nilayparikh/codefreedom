...
def _generate_codex_config(
    ...
    """Generate codex config.toml content pointing at the proxy.

    Codex discovers models dynamically from the provider's ``/v1/models``
    endpoint, so we only need to define the provider and default model.

    Key config fields:
    - model_provider: selects which provider to use
    - model: default model name
    - wire_api: "chat" for /v1/chat/completions (LiteLLM proxy format)
    - env_key: which env var holds the API key for authentication
    """
