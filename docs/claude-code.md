...
| ----------- | ---------------------- | ------------------------------------------------------------------ |
| `default`   | `CodeFreedom/Flash`    | Base profile -- routes through proxy, sets all model defaults       |
| `bare`      | _(none)_               | Minimal -- routes through proxy, no model aliases or preferences   |
| `ultra`     | `CodeFreedom/Ultra`    | Inherits from `default` -- best reasoning for architecture/planning |
| `pro`       | `CodeFreedom/Pro`      | Inherits from `default` -- balanced for bounded implementation      |
| `air`       | `CodeFreedom/Air`      | Inherits from `default` -- lightweight for mechanical tasks         |

The model aliases (`CodeFreedom/Flash`, `CodeFreedom/Pro`, `CodeFreedom/Ultra`, `CodeFreedom/Air`) are defined in the [proxy configuration](proxy/config.md#model-aliases), not in profiles.

---

For the complete profile reference -- inheritance mechanics, variable interpolation, tool declarations, sandbox images, tool profiles, and standardization guidelines -- see [Profiles](claude-code/profiles.md).
