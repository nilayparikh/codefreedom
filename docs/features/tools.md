...
Every tool follows the same four commands:

```bash
codefreedom tools <tool> init    # Set up (requires acceptance)
...
Each tool has a profile in `~/.codefreedom/profiles/<tool>.json` controlling its Docker settings (image, port, data directory). Generated by `init`, editable afterward.
