...
| -------------------------- | ------------------------------ | ---------------------------------------------- |
| [Local (Native)](local.md) | `codefreedom claude`           | Running on your host, no isolation needed      |
| [Sandbox](sandbox.md)      | `codefreedom claude --sandbox` | Isolated Docker container with GPU passthrough |
...
See [Sandbox Mode -> Available Images](sandbox.md#available-images) for the full tag reference and Dockerfile examples.
...
The model aliases (`${MODEL_NAME}`, `${MODEL_NAME_PRO}`, `${MODEL_NAME_ULTRA}`, `${MODEL_NAME_AIR}`) are defined in the [proxy configuration](../reference/proxy/config.md#model-aliases), not in profiles.

---

For the complete profile reference -- inheritance mechanics, variable interpolation, schema validation, tool declarations, sandbox images, and tool profiles -- see [Profiles](profiles.md).

For sandbox isolation details -- container lifecycle, file system isolation, network isolation, GPU passthrough, and security trade-offs -- see [Sandbox Isolation](sandbox-isolation.md).

See [Environment Configuration](../reference/environment.md) for the full env chain and variable interpolation details.
